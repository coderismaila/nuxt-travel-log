...
import { findLocationByNameAndUser, findUniqueSlug, insertLocation } from "~/lib/db/queries/location";
...
export default defineAuthenticatedEventHandler(async (event) => {
  ...
  const existingUserLocation = await findLocationByNameAndUser(result.data, event.context.user.id);
